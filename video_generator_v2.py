...
from src.images.image_validator import ImageValidator
from tts_engine import tts, COOKIES as TTS_COOKIES
from video_assembler_fast import FastVideoAssembler, VideoScene
    ...
    def _process_assets_manifest(self, scenario: dict, assets_dir: str) -> dict:
        ...
        for asset in normalized_manifest:
            ...
            if asset_type == "person":
                prefix = f"person_{asset_name.replace(' ', '_')}"
                result = self._search_person_photo(asset_name, asset_desc, assets_dir, prefix)
                if result:
            ...
            if not asset_found:
                ...
                if not asset_found and self.use_real_photos and self.has_image_search and "real" in self.image_search.services:
                    try:
                        search_query = asset_desc or asset_name
                        print(f"   📸 Поиск реального фото (DuckDuckGo): {search_query}")
            ...
            try:
                ...
                if bg_type == "person_photo":
                    ...
                    result = self._search_person_photo(
                        person_name, bg_prompt, assets_dir,
                        f"bg_person_{i+1}"
                    )
                    if result:
...
def main():
    ...
    parser.add_argument("--no-validate", action="store_true", help="Не валидировать изображения через Qwen VL")
